...
# 创建打包目录
output_dir = "Momo自动回复便携版"
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# 复制主程序 (注意这里源文件改为了 wechat_gui_momo.exe)
shutil.copy("dist/wechat_gui_momo.exe", f"{output_dir}/Momo自动回复.exe")

# 复制说明文件 (如果你有专门的momo说明文件，可以替换这里的名字)
if os.path.exists("打包说明.txt"):
...
with open(f"{runtime_dir}/VC++运行库下载地址.txt", "w", encoding="utf-8") as f:
    ...
    f.write("2. 安装完成后再运行 Momo自动回复.exe\n")

# 压缩成zip包
zip_name = "Momo自动回复便携版.zip"
...
print(f"便携版打包完成！生成文件：{zip_name}")
print("使用方法：")
print(f"1. 解压 {zip_name}")
print("2. 先安装运行库目录下的VC++运行库")
print("3. 然后运行 Momo自动回复.exe")